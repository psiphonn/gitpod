...
function ContextMenu(props: ContextMenuProps) {
    ...
    return (
        <div className="relative cursor-pointer">
            ...
            {expanded ?
                <div className={`mt-2 z-50 ${props.width || 'w-48'} bg-white dark:bg-gray-900 absolute right-0 flex flex-col border border-gray-200 dark:border-gray-700 rounded-lg truncated`}>
                    {props.menuEntries.map((e, index) => {
                        const clickable = e.href || e.onClick || e.link;
                        const entry = <div className={`px-4 flex py-3 ${clickable ? 'hover:bg-gray-200 dark:hover:bg-gray-700' : ''} text-sm leading-1 ${e.customFontStyle || font} ${e.separator ? ' border-b border-gray-200 dark:border-gray-700' : ''}`} >
